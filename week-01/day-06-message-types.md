...
在 Claude Code 中，一条"消息"可能包含文字、图片、工具调用、工具结果、进度更新、系统通知。`types/message.ts` 定义了这个丰富的消息系统：
...
2. **Tool Use 消息格式**
   - 🔗 https://docs.anthropic.com/en/docs/build-with-claude/tool-use#tool-use-and-tool-result-content-blocks
   - 推荐：工具调用的详细格式规范
...
2. **应用：** 如果一次助手消息中有 3 个工具调用，它们应该并行执行还是串行执行？
3. **思辨：** Claude Code 用内部消息格式再转换为 API 格式，为什么不直接用 API 格式？
